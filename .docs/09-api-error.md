...
従業員一覧の GET 失敗、従業員詳細・更新の GET 失敗、従業員追加の POST 失敗、従業員更新の PUT 失敗、従業員削除の DELETE 失敗で、共通のエラーコンポーネントを表示する。
...
PUT 失敗時は更新フォームを出さない。項目エラーは従来どおりフォーム上に出す。
